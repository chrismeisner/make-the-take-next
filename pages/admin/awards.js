...
export default function AwardsAdminPage() {
  ...
  const [form, setForm] = useState({ name: '', tokens: 25, code: '', redirectTeamSlug: '' });
  ...
  const create = async (e) => {
    ...
    try {
      const res = await fetch('/api/admin/awards/create', {
        ...
        body: JSON.stringify({ name: form.name, tokens: Number(form.tokens), code: form.code || undefined, redirectTeamSlug: form.redirectTeamSlug || undefined }),
      });
      const data = await res.json();
      if (res.ok && data.success) {
        setForm({ name: '', tokens: 25, code: '', redirectTeamSlug: '' });
        await load();
      } else {
    ...
    <div className="p-4">
      ...
      <section className="border rounded-lg p-4 mb-6">
        ...
        <form onSubmit={create} className="flex flex-wrap items-end gap-3">
          ...
          <div>
            <label className="block text-sm text-gray-700">Redirect Team (slug)</label>
            <input value={form.redirectTeamSlug} onChange={(e) => setForm({ ...form, redirectTeamSlug: e.target.value })} className="mt-1 px-3 py-2 border rounded" placeholder="e.g. nyy" />
          </div>
