...
export default async function handler(req, res) {
  ...
  const { name, tokens, code, validFrom, validTo, redirectTeamSlug } = req.body || {};
  ...
  const sql = `INSERT INTO award_cards (code, name, tokens, valid_from, valid_to, redirect_team_slug) VALUES ($1,$2,$3,$4,$5,$6) RETURNING code`;
  const genCode = providedCode || (Math.random().toString(36).slice(2, 10) + Math.random().toString(36).slice(2, 10)).slice(0, 12);

  try {
    const { rows } = await query(sql, [genCode, name, safeTokens, validFrom || null, validTo || null, redirectTeamSlug || null]);
