...
export default async function handler(req, res) {
  ...
  try {
    ...
    // 0.6) Upsert Profiles and build map from Airtable profile record id -> profiles.id
    const profileRecords = await base('Profiles').select({ pageSize: 100, view: 'Grid view' }).all();
    const profileRecMap = new Map();
    const profileIdTextToRowId = new Map();
    for (const rec of profileRecords) {
      const f = rec.fields || {};
      const profileID = f.profileID || null;
      const mobile = f.profileMobile || null;
      const username = f.profileUsername || null;
      let favoriteTeamId = null;
      try {
        const favLink = Array.isArray(f.profileTeam) ? f.profileTeam[0] : f.profileTeam;
        if (favLink && teamRecMap.has(favLink)) favoriteTeamId = teamRecMap.get(favLink);
      } catch {}
      const upsertSql = `
        INSERT INTO profiles (profile_id, mobile_e164, username, favorite_team_id)
        VALUES ($1,$2,$3,$4)
        ON CONFLICT (profile_id) DO UPDATE SET
          mobile_e164 = COALESCE(EXCLUDED.mobile_e164, profiles.mobile_e164),
          username = COALESCE(EXCLUDED.username, profiles.username),
          favorite_team_id = COALESCE(EXCLUDED.favorite_team_id, profiles.favorite_team_id)
        RETURNING id`;
      const { rows } = await query(upsertSql, [profileID, mobile, username, favoriteTeamId]);
      if (rows[0]?.id) {
        report.profiles.upserted += 1;
        if (favoriteTeamId) report.profiles.linkedTeams += 1;
        profileRecMap.set(rec.id, rows[0].id);
        if (profileID) profileIdTextToRowId.set(profileID, rows[0].id);
      }
    }
