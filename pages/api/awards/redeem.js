...
export default async function handler(req, res) {
  ...
  try {
    ...
    return res.status(200).json({ success: true, code: result.code, name: result.name, tokens: Number(result.tokens) || 0, redirectTeamSlug: result.redirect_team_slug || null, already });
  } catch (err) {
