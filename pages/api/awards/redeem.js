...
export default async function handler(req, res) {
  ...
  if (rec.requirement_key === 'follow_team' && rec.requirement_team_slug) {
    try {
      ...
      const currentSlug = rows?.[0]?.team_slug || null;
      const matches = currentSlug && rec.requirement_team_slug
        ? String(currentSlug).toLowerCase() === String(rec.requirement_team_slug).toLowerCase()
        : false;
      if (!matches) {
        // Provide friendly team name if possible
        const teamName = rows?.[0]?.name || null;
        return res.status(400).json({ success: false, error: 'Requirement not met', code: 'requirement_follow_team', requirementKey: rec.requirement_key, requirementTeamSlug: rec.requirement_team_slug, requirementTeamName: teamName });
      }
    } catch (e) {
