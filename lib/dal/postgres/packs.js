...
export class PostgresPacksRepository extends PacksRepository {
  ...
  async createOne(data) {
    const { packURL, packTitle, packSummary, packType, packLeague, packStatus, packOpenTime, packCloseTime, packCoverUrl, prize, prizeSummary, packPrizeURL, eventId, events, packID, packOpenSmsTemplate, creatorProfileId, dropStrategy } = data || {};
    console.log('[PG PacksRepository.createOne] incoming times =>', { packOpenTime, packCloseTime });
    try {
      console.log('[PG PacksRepository.createOne] creating pack', {
        packURL,
    ...
    const { rows } = await query(
      `INSERT INTO packs (pack_url, title, summary, pack_status, league, cover_url, prize, event_id, pack_id, pack_open_time, pack_close_time, pack_open_sms_template, creator_profile_id, drop_strategy)
       VALUES ($1,$2,$3,$4,$5,$6,$7,$8, COALESCE($9, LEFT(ENCODE(gen_random_bytes(9),'hex'),12)),$10,$11,$12,$13,$14)
       ON CONFLICT (pack_url) DO UPDATE SET
         ...
         drop_strategy = COALESCE(EXCLUDED.drop_strategy, packs.drop_strategy)
       RETURNING *`,
      [packURL, packTitle || null, packSummary || null, packStatus || null, packLeague || null, packCoverUrl || null, prize || null, eventId || null, packID || null, packOpenTime || null, packCloseTime || null, packOpenSmsTemplate || null, creatorProfileId || null, dropStrategy || null]
    );
    const created = this.#map(rows[0]);
    try {
      console.log('[PG PacksRepository.createOne] created', {
        id: created?.id,
        packURL: created?.packURL,
        coverUrl: created?.packCover?.[0]?.url || null,
      });
    } catch {}
  ...
  async updateByPackURL(packURL, fields) {
    const allowed = ['title','summary','pack_status','league','cover_url','prize','event_id','pack_open_time','pack_close_time','pack_open_sms_template','drop_strategy'];
    const sets = [];
    const vals = [];
    let i = 1;
  ...
  #map(r) {
    return {
      ...
      dropStrategy: r.drop_strategy || 'link',
