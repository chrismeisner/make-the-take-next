...
export class PostgresPacksRepository extends PacksRepository {
  ...
  async createOne(data) {
    const { packURL, packTitle, packSummary, packType, packLeague, packStatus, packOpenTime, packCloseTime, packCoverUrl, prize, prizeSummary, packPrizeURL, eventId, events, packID } = data || {};
    console.log('[PG PacksRepository.createOne] incoming times =>', { packOpenTime, packCloseTime });
    const { rows } = await query(
      `INSERT INTO packs (pack_url, title, summary, pack_status, league, cover_url, prize, event_id, pack_id, pack_open_time, pack_close_time)
       VALUES ($1,$2,$3,$4,$5,$6,$7,$8, COALESCE($9, LEFT(ENCODE(gen_random_bytes(9),'hex'),12)),$10,$11)
       ...
       RETURNING *`,
      [packURL, packTitle || null, packSummary || null, packStatus || null, packLeague || null, packCoverUrl || null, prize || null, eventId || null, packID || null, packOpenTime || null, packCloseTime || null]
    );
  ...
  async updateByPackURL(packURL, fields) {
    const allowed = ['title','summary','pack_status','league','cover_url','prize','event_id','pack_open_time','pack_close_time'];
    const sets = [];
    const vals = [];
    let i = 1;
