...
export class PostgresTeamsRepository extends TeamsRepository {
  async listAll() {
    ...
    return rows.map((r) => ({
      ...
      teamAbbreviation: r.abbreviation || r.team_slug || '',
      teamLeague: r.league || '',
  ...
  async createOne(data) {
    ...
    const { rows } = await query(
      `INSERT INTO teams (team_id, name, short_name, team_slug, abbreviation, league, emoji, logo_url, team_home_side, team_away_side)
  ...
  async updateOne(idOrTeamID, fields) {
    ...
    if (fields.teamAbbreviation !== undefined) { sets.push(`abbreviation = $${i++}`); values.push(fields.teamAbbreviation); }
    ...
    console.log('[DAL/PostgresTeamsRepository.updateOne] Applying update', {
      ...
      updates: {
        ...
        abbreviation: fields.teamAbbreviation !== undefined ? fields.teamAbbreviation : undefined,
